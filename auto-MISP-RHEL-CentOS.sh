...
openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 -subj "/C=SN/ST=Dakar/L=Dakar/O=Security/CN=$FQDN" -keyout /etc/pki/tls/private/$FQDN.key  -out /etc/pki/tls/certs/$FQDN.crt &>> $logfile
...
$SUDO_WWW $RUN_PHP -- $CAKE Admin setSetting "Plugin.Enrichment_pdf_enrich_enabled" true &>> $logfile
error_check 'enrichment setSetting command 7'
$SUDO_WWW $RUN_PHP -- $CAKE Admin setSetting "Plugin.Enrichment_docx_enrich_enabled" true &>> $logfile
error_check 'enrichment setSetting command 8'
$SUDO_WWW $RUN_PHP -- $CAKE Admin setSetting "Plugin.Enrichment_xlsx_enrich_enabled" true &>> $logfile
error_check 'enrichment setSetting command 9'
$SUDO_WWW $RUN_PHP -- $CAKE Admin setSetting "Plugin.Enrichment_pptx_enrich_enabled" true &>> $logfile
error_check 'enrichment setSetting command 10'
$SUDO_WWW $RUN_PHP -- $CAKE Admin setSetting "Plugin.Enrichment_ods_enrich_enabled" true &>> $logfile
error_check 'enrichment setSetting command 11'
$SUDO_WWW $RUN_PHP -- $CAKE Admin setSetting "Plugin.Enrichment_odt_enrich_enabled" true &>> $logfile
...
$SUDO_WWW $RUN_PHP -- $CAKE Admin setSetting "updateDatabase" &>> $logfile
